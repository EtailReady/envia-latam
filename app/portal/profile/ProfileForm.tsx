...
export default function ProfileForm({ initialData }: { initialData: ProfileData }) {
  ...
  const labelStyle: React.CSSProperties = { display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'var(--dark)', marginBottom: '5px' };
  const sectionHead: React.CSSProperties = { fontSize: '0.78rem', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '14px', paddingBottom: '10px', borderBottom: '1px solid var(--border)' };
    ...
    <div style={{ padding: 'clamp(24px, 4vw, 40px)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '28px', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          {profile.courierSlug && (
            <Link
              href={`/courier/${profile.courierSlug}`}
              target="_blank"
          ...
          <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', background: 'var(--bg)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem', color: 'var(--muted)' }}>
                <Info size={12} /> Cambios visibles al guardar
              </div>
            </div>
          </div>
        </div>
